...
**検証（2026-09-03 実機・15インチAir）**
- `swift build -c release` 成功（Swift 6.3.3 / arm64-apple-macosx26.0、47.4秒）。署名検証も通過。`/Applications/PokecaRecords_v1.20.app` へ自動配置された。残った警告は209行目の `try?` 未使用1件のみで、これは今回触っていないログ書き出し部分・v1.19以前から出ていたもの。
- 起動して入力画面を確認。日時欄が `2026-09-03 11:27:47` → `11:27:53` と秒まで進むことを画面で確認（自動モード、キャプション「保存を押した時刻で記録します」）。
- 「日時を指定する」で手動モードへ切り替わり、DatePickerとステッパー・「今にする」・オレンジのキャプション「手で指定した日時で記録します」が出ることを確認。「今にする」で自動へ戻ることも確認。
- 戦績一覧を開き、既存データが正常に読めることを確認。**日時読み取り警告バナーは出ていない**＝207件すべて既存フォーマットで問題なくパースされている（寛容パーサと`.distantPast`分岐は発火していない）。
- **未確認**: 実際に保存を押したときの時刻一致は、本人の次の実戦入力で確認する（検証用の戦績をDBへ書き込まないため、こちらでは保存していない）。
