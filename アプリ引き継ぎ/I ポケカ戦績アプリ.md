...
#### 2026-09-12 v1.21修正（英字入力・相手候補順）

- 本人からレート欄の英字切り替えが外れたようだとの報告。既存のTIS切り替えは残っていた。マウスから共有フィールドエディタへ入る経路を補強し、`RomanNSTextField.textDidBeginEditing`でも切り替え、`currentEditor()?.inputContext?.allowedInputSourceLocales = ["en"]`を設定。終了時に制限をnilへ戻して以前の入力ソースを復元する。元の症状の発生条件そのものは未特定。
- 相手デッキ候補だけ、固定候補を先頭に保ち、11回以上は回数降順、その後の10回以下（0件含む）は日本語ロケールで名前順。候補ボタンと「選択」メニューに共通適用。自分のデッキの並びは従来どおり。
- バンドルID・版名はv1.21を維持。ソース、Vaultの.app、`/Applications/PokecaRecords_v1.21.app`を更新し署名確認済み。旧.appは `/private/tmp/PokecaRecords_v1.21-before-20260912.app` に退避。
- **最終確認: 2026-09-12**。修正版を再起動し、相手候補の11回までが先頭、その後が「Nのバイバニラ、アブソル、イイネイヌ、イーブイズ、イワパレス…」の順になることを実画面で確認。10/11境界・固定候補・自分のデッキ従来順は抽出した関数でも検査。
- 入力検証: 相手デッキ欄でaキー→「あ」、マウスでレート欄に移りaキー→「a」、相手デッキへ戻ってaキー→「あ」を確認。テスト入力を消し、レートは元の1347へ戻した。検証用の戦績は保存していない。
- ★ビルド環境の地雷: 9月11日更新のCLTで既定SDKがmacOS27となり、既存の`@State`が`SwiftUIMacros`プラグイン不足でコンパイル不可。コードの問題として書き換えず、同居するMacOSX26.5.sdkを`swift build --sdk`へ渡すことでビルド・実起動まで成功。`build.command`にも26.5 SDKが存在する場合の明示指定を追加。今回のビルドログは `/private/tmp/pokeca-build26.log`。

#### 2026-09-12 追補（大きい保存ボタン・候補の境目）

- 本人希望で保存ボタンを180×112ptの大きな色付き面にした。標準`.borderedProminent`による高さ制約を避け、ラベルに背景・角丸・contentShapeを付けて`.plain`で描く。勝敗色、Returnキー、保存/更新の動作は維持。
- 相手候補の11回以上は回数順を維持。最初の固定されていない10回以下の候補の前に縦線と「10回以下・アイウエオ順」を追加。「選択」メニューにも同位置に区切りと見出しを追加。
- 最終確認: 2026-09-12。26.5 SDKでビルドし、Vaultと/Applicationsのv1.21を更新・再起動。実画面の画像で大きい緑の保存ボタンを確認。AXでメガドリューズ11回→境目のテキスト→Nのバイバニラ1回からの名前順を確認。候補列は従来どおり横スクロールのため、境目は右側にある。
