...
public final class CaptureLibrary {
    ...
    public func setDate(_ date: String?, for id: UUID, expectedRevision: Int) throws {
        try validateDate(date)
        try locked {
            try transaction {
                let existing = try getUnlocked(id)
                guard try rows("SELECT capture_id FROM adoptions WHERE capture_id=?", [id.uuidString]).isEmpty else {
                    throw DiaryError.invalid("日記へ保存した文章の日付変更は準備中です。保存済みの日付を保持します。")
                }
    ...
    private func recoverUnlocked() throws {
        if allowJournalWrites { try recoverAdoptionsUnlocked() }
