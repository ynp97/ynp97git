...
struct BackupSnapshot: Codable, Equatable {
    ...
    static let roots = ["ジャーナル", "journal", "media", "日記アプリデータ/originals", "日記アプリデータ/operations", "日記アプリデータ/staging"]
    ...
    func copy(from source: URL, to destination: URL) throws {
        let fm = FileManager.default
        for path in Self.roots where FileManager.default.fileExists(atPath: source.appendingPathComponent(path).path) {
            try fm.createDirectory(at: destination.appendingPathComponent(path), withIntermediateDirectories: true)
        }
        for path in files.keys.sorted() {
            ...
            try DurableFiles.syncFile(target)
        }
    }
}
