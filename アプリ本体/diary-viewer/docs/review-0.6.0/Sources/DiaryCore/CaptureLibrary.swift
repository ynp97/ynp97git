...
public final class CaptureLibrary {
    ...
    private func backupUnlocked(to destination: URL, interruption: ((BackupStage) throws -> Void)? = nil) throws {
            guard try rows("SELECT id FROM captures WHERE phase='pending'").isEmpty else {
            ...
            do {
                ...
                if let payload = try rows("SELECT payload FROM journal_identity WHERE singleton=1").first?.first {
                    let identity = try JSONDecoder().decode(JournalIdentity.self, from: Data(payload.utf8))
                    let directory = fm.fileExists(atPath: temporary.appendingPathComponent("ジャーナル").path) ? "ジャーナル" : "journal"
