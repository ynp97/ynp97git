...
struct JournalParser {
    ...
    static func parse(fileContent: String, year: Int, persistentIDs: [UUID]? = nil) -> ParseResult {
        do {
            let document = try OriginalJournal(data: Data(fileContent.utf8))
            if let persistentIDs {
                guard persistentIDs.count == document.blocks.count, Set(persistentIDs).count == persistentIDs.count else {
                    throw DiaryError.invalid("日記IDの対応件数または重複に問題があります。")
                }
                for (index, block) in document.blocks.enumerated() {
                    if let explicit = block.explicitID, explicit != persistentIDs[index] {
                        throw DiaryError.invalid("明示IDと対応表が一致しません。")
                    }
                }
            }
            var entries: [Entry] = []
            for (index, block) in document.blocks.enumerated() {
                if let record = try PlainJournalRecord.decode(block.data) {
                    ...
                    if let id = persistentIDs?[index] ?? block.explicitID { entry.id = id }
                    entries.append(entry)
                }
            }
