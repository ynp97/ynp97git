...
public struct OriginalJournal {
    ...
    /// 年移動・受け箱へ戻す際にも、他の記録とヘッダの独自項目を再構成しない。
    public func removingBlock(at index: Int, expectedHash: String) throws -> Data {
        try validateForWriting()
        guard contentHash(data) == expectedHash else { throw DiaryError.conflict }
        guard blocks.indices.contains(index), let boundary = blocks.first?.range.lowerBound else {
            throw DiaryError.invalid("移動する日記がありません。")
        }
        let prefix = String(decoding: data.prefix(boundary), as: UTF8.self)
        let pattern = try NSRegularExpression(pattern: #"(?m)^(\s*entries:[ \t]*)([0-9]+)([ \t]*\r?)$"#)
        let matches = pattern.matches(in: prefix, range: NSRange(prefix.startIndex..., in: prefix))
        ...
        var header = prefix
        header.replaceSubrange(range, with: String(blocks.count - 1))
        var body = data.subdata(in: boundary..<data.count)
        let removed = blocks[index].range
        body.removeSubrange((removed.lowerBound - boundary)..<(removed.upperBound - boundary))
        var output = Data(header.utf8)
        output.append(body)
        let next = try OriginalJournal(data: output)
        try next.validateForWriting()
        let retained = blocks.enumerated().filter { $0.offset != index }.map(\.element)
        guard next.blocks.count == retained.count,
              zip(next.blocks, retained).allSatisfy({ $0.data == $1.data }) else {
            throw DiaryError.invalid("別の日記に変更が及ぶため移動しません。")
        }
        return output
    }
}
