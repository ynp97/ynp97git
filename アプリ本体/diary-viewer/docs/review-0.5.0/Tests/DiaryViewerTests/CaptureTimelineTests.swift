...
final class CaptureTimelineTests: XCTestCase {
    ...
    func testLegacyParserUsesPersistentMapAndRejectsMismatch() throws {
        let content = "## 2026-01-01（木）1:00:00\n本文A\n\n## 2026-01-02（金）2:00:00\n本文B\n"
        let ids = try CaptureLibrary(root: root).journalIdentities(files: ["2026.md": Data(content.utf8)])["2026.md"]!
        let result = JournalParser.parse(fileContent: content, year: 2026, persistentIDs: ids)
        XCTAssertNil(result.failure)
        XCTAssertEqual(result.entries.map(\.id), ids)
        XCTAssertEqual(JournalParser.parse(fileContent: content, year: 2026, persistentIDs: ids).entries, result.entries)
        XCTAssertTrue(JournalParser.parse(fileContent: content, year: 2026, persistentIDs: [ids[0]]).failure != nil)
        XCTAssertTrue(JournalParser.parse(fileContent: content, year: 2026, persistentIDs: [ids[0], ids[0]]).failure != nil)
    }
